...
#Crowdsale
@public
def __init__(_rate: uint256, _wallet: address, _token: address, _cap: uint256):
    """
...
def processTransaction(_sender: address, _beneficiary: address, _weiAmount: uint256(wei)):
    ...
    assert as_unitless_number(self.weiRaised + _weiAmount) <= self.cap, "Sorry but this exceeds the cap."
