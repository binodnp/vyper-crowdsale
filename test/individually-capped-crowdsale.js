...
const CappedCrowdsale = artifacts.require('individually_capped_crowdsale.vyper');
...
contract('IndividuallyCappedCrowdsale', function ([_, wallet, alice, bob, charlie]) {
  ...
  describe('individual capping', function () {
    ...
    shouldBehaveLikeOwnable([_, wallet, alice, bob, charlie]);
  ...
  describe('group capping', function () {
    beforeEach(async function () {
      this.token = await SimpleToken.new(web3.fromAscii("Name"), web3.fromAscii("SYMBOL"), tokenSupply, 18);
      this.crowdsale = await CappedCrowdsale.new(rate, wallet, this.token.address);
      await this.crowdsale.setGroupCap(2, [bob, charlie], capBob);
      await this.token.transfer(this.crowdsale.address, tokenSupply);
    });
